...
public abstract class GesComCtrl <H extends BaseEntity, T extends H>{
	
	/***
	 * Vue par défaut d'une entité 
	 * Doit etre instanciée par la classe concrète 
	 */
	public GesComVue<T> defaultVue;
	 ...
	 */
	public GesComVue<T> getDefaultVue() {
		return defaultVue;
	}
	 ...
	 */
	public void setDefaultVue(GesComVue<T> defaultVue) {
...
//			v$entiteRecherche.setMaxRow(pasPagination);	
	...
	public abstract List<T>  rechercherParCritere(T p$entity) throws GesComAppException;
	...
	public String selectionner(){
		...
		if(defaultVue.getNavigationMgr().getEnSelection()){
			...
			GesComCtrl<H,T> v$controlleur  =  (GesComCtrl<H,T>) FacesUtil.getSessionMapValue(defaultVue.getNavigationMgr().getSelectionBeanCtrlName());
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void refreshData(GesComCtrl p$controleur){
