...
public class SisvAutSpeImp extends BaseSisv<TabAutSpeImp, String> implements ISisvAutSpeImp{
	...
	public <X extends BaseEntity> X creer(X p$entite) throws BaseException {
		try {
			...
			String codAut =  DateTools.getYear(DateTools.formatDate(new Date()))+"AI" + numAut + infUsr.getUser().getCodSite();
	...
	public  List<TabPro> rechercherProduitParCodAut(String codAut)
			throws GesComSystemException {
		try {
			return daoAutSpe.findProductByCodAut(codAut);
		} catch (GesComPersistenceException e) {
			e.printStackTrace();
			GesComSystemException sbr = new GesComSystemException(e);
			throw sbr;
...
//		 numero = DateTools.getYear(DateTools.formatDate(new Date())) + numero;
	...
	private SrlImportation getEtatImportation(TabAutSpeImp v$TabAutImp) throws GesComSystemException {
		...
		try {
			...
			List<TabPro> listPro = daoAutSpe.findProductByCodAut(v$TabAutImp.getCodAut());
