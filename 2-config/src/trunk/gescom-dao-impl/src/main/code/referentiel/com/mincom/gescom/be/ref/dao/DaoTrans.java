...
@Stateless
public class DaoTrans extends BaseDao<TabTrans, String> implements IDaoTrans{
	
	private static BaseLogger logger = BaseLogger.getLogger(DaoTrans.class);
	...
	public <X extends BaseEntity> List<X> findByExample(X entity)
		...
		TabTrans currentTrans = (TabTrans)entity;
		if(currentTrans.getCodTrans()!= null && !currentTrans.getCodTrans().equals("")){
			clauseWhere = clauseWhere + "AND upper(o.codTrans) like '%"+currentTrans.getCodTrans()+"%'";
		}
		if(currentTrans.getLibTrans()!= null && !currentTrans.getLibTrans().equals("")){
			clauseWhere = clauseWhere + "AND upper(o.libTrans) like '%"+currentTrans.getLibTrans()+"%'";
		}
		try{
